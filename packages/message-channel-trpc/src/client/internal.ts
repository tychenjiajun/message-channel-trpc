import { TRPCClientRuntime, TRPCClientError } from '@trpc/client';
import { AnyRouter, inferRouterError } from '@trpc/server';
import { TRPCResponseMessage, TRPCResponse, TRPCResultMessage } from '@trpc/server/rpc';
...
export function transformResult<TRouter extends AnyRouter, TOutput>(
  ...
  if (
    !result.ok &&
    (!isObject(result.error.error) || typeof result.error.error.code !== 'number')
  ) {
